...
mpa         <- loadRData(file.path(spatialdir, "MPANLD3.RData")) %>% 
  # filter(SITE_NAME != "Bruine bank (noord)") %>% 
  # filter(!(SITE_NAME == "Friese Front" & SITE_TYPE == "N2000")) %>% 
  sf::st_as_sf()

ncp         <- eez_sf %>% filter(grepl("NLD", ISO_TER1)) %>% sf::st_as_sf()
eezs        <- eez_sf %>% filter(grepl("GBR|BEL|FRA|DEU|DNK", ISO_TER1)) %>% 
  dplyr::select(SITE_NAME=ISO_TER1) %>%
  mutate(SITE_TYPE="OPEN") %>% 
  sf::st_as_sf()

#Bruine bank
bb          <- mpa %>% filter(SITE_NAME=="Bruine Bank") %>% sf::st_as_sf()
bbn         <- mpa %>% filter(SITE_NAME=="Bruine bank (noord)") %>% sf::st_as_sf()
bbz         <- rmapshaper::ms_erase(target=bb, erase=bbn) %>% mutate(SITE_NAME="Bruine bank (zuid)")
...
mpa         <- bind_rows(mpa, non_mpa, bbz, eezs) %>% 
