...
setvessel<-c("Z99", "SCH99")   ;settrip<- c( "2025197") ; comparisons <- FALSE
...
if (sum(!is.na(t$landingweight)) >0 ) {
  ...
  t %>%
    ...
    geom_jitter(aes(x=lon, y=lat, size = landingweight, colour = as.character(date)),
...
if (nrow(m) > 0 & sum(!is.na(h$catchweight))>0) {
  ...
  fig_nums(
    ...
    caption = "Herekende percentage discards per dag")
...
ee %>% 
  ...
  scale_linewidth_manual("year", values = c("2020"=.5,       "2021"=.5,       "2022"=.5,       "2023"=0.5,      "2024"=1.5), guide = "none") +
  scale_colour_manual   ("year", values = c("2020"="gray80", "2021"="gray60", "2022"="gray40", "2023"="blue", "2024"="red")) +
